...
public interface BaseRepository<T> extends JpaRepository<T, Long> {
     ...
     */
    @Query("FROM #{#entityName} WHERE id = ?1 AND isDelete = 'false'")
     ...
     * @return Optional&lt;T&gt; record identified
     */
    @Query("FROM #{#entityName} WHERE code = ?1 AND isDelete = 'false'")
     ...
     */
    @Query("FROM #{#entityName} WHERE isDelete = '0' ORDER BY id DESC")
    Set<T> getAll();
     ...
     * @return Set&lt;T&gt; records
     */
    @Query("FROM #{#entityName} WHERE isDelete = false")
     ...
     * @return Optional&lt;T&gt; record identified
     */
    @Query("FROM #{#entityName} WHERE modifierPar = ?1 AND isDelete = false")
    ...
    @Modifying
    @Transactional
    @Query("UPDATE #{#entityName}  SET isDelete = true WHERE id = ?1")
    void unlink(Long id);
